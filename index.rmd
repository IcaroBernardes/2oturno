...
dados$data %>% purrr::walk(
  function(dados){
    ...
    p = plot +
      geom_path(aes(x = tempo, y = pct, color = I(cor)),
                size = 1, data = dados) +
      geom_point(aes(x = tempo, y = pct, color = I(cor)),
                 size = 7, data = dados) +
      ggtext::geom_richtext(aes(x = tempo, y = pct, label = label),
                            fill = "white", label.colour = NA,
                            label.margin = unit(c(0, 0, 0, 0.03), "npc"),
                            hjust = 0, size = 1.5, data = dados) +
      ggpath::geom_from_path(aes(x = tempo, y = pct, path = path),
                             width = 0.17, height = 0.17, data = dados) +
      ggtext::geom_richtext(aes(x = tempo, y = 40, label = glue("{labels}%")),
                            fill = "white", label.colour = NA,
                            size = 1, data = dados)
...
plot <- plot +
  ...
  gganimate::transition_reveal(along = tempo) +
